...
test('daily odds blend adds Odds Pts without changing model-only ranks, and falls back per missing term', () => {
  ...
  const noCoverage = poolForProfile(snapshot, FREE_KICK_GW1_SAT, placeholder);
  ...
  assert.equal(hasOddsCoverage(placeholder), false, 'an unpulled placeholder is not odds coverage');
  assert.deepEqual(noCoverage, plain, 'an unpulled placeholder behaves exactly like no asset');
});
