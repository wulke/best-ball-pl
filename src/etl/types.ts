...
/** Results-driven opponent strength (#92) — sufficient statistics, not
 *  multipliers: per-club season-to-date attack/concession sums on ONE
 *  source's scale, plus that scale's league mean. The model derives shrunk
 *  multipliers at projection time (knobs live in config.ts), so calibration
 *  changes never force an ETL re-run. `source` declares the scale — xG-scale
 *  sums use the xG shrinkage constants, goal-scale the heavier ones from
 *  docs/research/opponent-strength-signals.md. Omitted entirely until the
 *  first current-season match is played (pre-season parity: the model runs
 *  its FDR-only path bit-for-bit). */
export type SnapshotStrength = {
  source: 'understat' | 'fixture-goals';
  /** League-wide mean of the attack signal per team-match on this source's
   *  scale — the 1.0 the multipliers normalize against. */
  leagueAttackPerMatch: number;
  /** Date of the last match the aggregates count (source's own format —
   *  informational; Understat rows are not GW-gated). */
  through: string;
...
export type Snapshot = {
  ...
  /** Results-driven opponent strength (#92) — omitted until a match is played. */
