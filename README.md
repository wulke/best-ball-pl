...
- **Correlation badges (#117/#120, daily slates)**: stack/anti-stack rules live in `src/stacking/rules.ts` (`Rule` = `{slots, direction, scope, weight}` + `appliesTo` for candidate-position gates). Once your roster holds a **G+D clean-sheet pair from club X**, two things flag the other side of that fixture: a `⚔` badge on every **MD/FW from X's opponent** in the Sheet (tooltip names your stack club — their attacking returns vanish exactly when your pair's clean sheet pays), and a `⚔ {opponent} attackers` warning chip in the Live panel's correlation watch. The `⚔ vs {club}` tag also appears on such players inside the panel's recommendation rows. Informational only — no effect on ranking or scores; clubs with no window fixture never warn. False Nine mounts none of it (no window fixtures to oppose).
