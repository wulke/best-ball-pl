...
| `npm run positions:audit` | Diff **Underdog's own position tags** (the `MD - NFO` lines in every committed draft capture under `data/drafts/`) against the snapshot's FPL-derived positions and print override-ready entries for `src/etl/position-overrides.ts` — report only; picks are trusted only when the recap's club agrees with the snapshot club, conflicting tags and club mismatches are quarantined for eyes-on review |
...
6. After the slate's drafts, commit the captures and run `npm run positions:audit` — new players you saw tagged differently in the draft room surface as override candidates (the season's Underdog tags are locked, so every entry is permanent for the season).
...
**Underdog position corrections (#60, #132)**: player positions default to FPL's `element_type`, then `npm run etl` applies the checked-in table in `src/etl/position-overrides.ts` (34 audited entries as of 2026-08-27 — the wide-attacker wave where Underdog tags FPL mids as FW, plus Dorgu/Kroupi-style disagreements in both directions). The maintenance loop is capture-driven: every committed draft recap carries Underdog's own position-club line per pick (stored on the pick as `udPosition`/`udClub`), so after committing new captures run `npm run positions:audit` — it prints override-ready entries (with capture provenance and pick counts) for everything Underdog disagrees on, flags conflicting tags and club-mismatched joins for eyes-on confirmation in the Underdog app, and lists already-overridden players the captures confirm. Paste the trusted entries into the table, `npm run etl`, commit the refreshed snapshot, PR. Entries always carry a dated source note (#58's audit protocol); overrides feed roster legality, position ranks, scarcity, and the model's position baselines — exactly the contest semantics Underdog drafts on.

Permanence and coverage (#132): the table lives in code and `npm run etl` re-applies it on every run — the daily scheduled refresh cannot wash the corrections out of the snapshot (a CI guard asserts the committed snapshot reflects every override whose player is in the pool, and the ETL's Positions line reports `N Underdog overrides applied` so every refresh PR shows it). Season-long Underdog tags are effectively frozen once drafts open, so entries never go stale mid-season. Coverage grows with captures: a player defaults to FPL's tag until a committed capture observes Underdog's (or you eyeball it in the app). The known gaps are wide attackers nobody has drafted yet and January-window signings — both resolve the first time they appear in a capture.
...
- **Recent role signal (#134)**: `src/model/role.ts` derives a separate, deliberately capped three-team-GW window from retained actuals: recency-weighted start probability (60/25/15) and minutes per appearance. Postponed/rest GWs can be excluded with fixture context; unused-sub GWs lower the start probability but never dilute the conditional minutes signal. This pure derivation is not wired into projections yet.
...
- **Correlation badges (#117/#120, daily slates)**: stack/anti-stack rules live in `src/stacking/rules.ts` (`Rule` = `{slots, direction, scope, weight}` + `appliesTo` candidate gate + `prospectiveTooltip`). Once your roster holds a **G+D clean-sheet pair from club X**, every **MD/FW from X's opponent** carries a red `⚔` badge in the Sheet (tooltip names your stack club — their attacking returns vanish exactly when your pair's clean sheet pays), a `⚔ {opponent} attackers` chip joins the Live panel's correlation watch, and a `⚔ vs {club}` tag marks such players inside the panel's recommendation rows. A **half-held stack** (you hold X's GK or DEF alone) flags the same attackers in a **lighter tone** instead — blue `⚔` badge + `½⚔ vs {club}` tag, no panel chip — a heads-up, not a warning, so "DEF from A, FW from B" stays a normal pick. Informational only — no effect on ranking or scores; clubs with no window fixture never warn. False Nine mounts none of it (no window fixtures to oppose).
